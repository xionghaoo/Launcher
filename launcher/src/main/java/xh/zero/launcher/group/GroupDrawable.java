...
//    private float _scaleFactor = 1;
    ...
    public GroupDrawable(Context context, DesktopItem item, int iconSize) {
        final float size = iconSize;
        ...
        for (int i = 0; i < 4 && i < item.getItems().size(); i++) {
            DesktopItem temp = item.getItems().get(i);
            App app = null;
            if (temp != null) {
                app = AppLoader.Companion.instance(context).findApp(temp.getIntent());
            }
            if (app == null) {
